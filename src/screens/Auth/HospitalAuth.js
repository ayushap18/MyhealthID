...
const HOSPITAL_IMAGE = 'https://images.unsplash.com/photo-1582719478250-c89cae4dc85b?auto=format&fit=crop&w=640&q=80';

const HospitalAuth = ({ navigation }) => {
  ...
  const validateForm = () => {
    ...
    if (!isLogin) {
      if (!formData.name || formData.name.trim().length < 2) {
        newErrors.name = 'Hospital/Staff name is required';
      }
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (data.user.role !== 'hospital' && data.user.role !== 'doctor') {
        await apiService.auth.logout();
        Alert.alert(
          'Wrong Portal',
          'This account is registered as ' + data.user.role + '. Please use the correct login portal.',
          [{ text: 'OK' }]
        );
        return;
      }

      navigation.reset({ index: 0, routes: [{ name: 'HospitalDashboard' }] });
    } catch (error) {
      logger.error('HospitalAuth', 'Login error', error);
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (data.wallet) {
        Alert.alert(
          'Healthcare Account Created!',
          'Wallet: ' + data.wallet.address.slice(0, 20) + '...\n\nRecovery Phrase:\n' + data.wallet.mnemonic + '\n\nSave this securely!',
          [{ text: 'I Have Saved It', onPress: () => navigation.reset({ index: 0, routes: [{ name: 'HospitalDashboard' }] }) }]
        );
      } else {
        navigation.reset({ index: 0, routes: [{ name: 'HospitalDashboard' }] });
      }
    } catch (error) {
      logger.error('HospitalAuth', 'Register error', error);
    ...
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        <Animated.View style={[styles.header, { opacity: fadeAnim, transform: [{ translateY: slideAnim }, { scale: headerScale }] }]}>
          ...
          <Text style={styles.subtitle}>{isLogin ? 'Staff login portal' : 'Register healthcare account'}</Text>
        </Animated.View>
        ...
        <Animated.View style={[styles.form, { transform: [{ translateX: formSlide }] }]}>
          {!isLogin && (
            <View style={styles.inputGroup}>
              <Text style={styles.label}>Hospital / Staff Name</Text>
          ...
          <Animated.View style={[styles.infoBox, { opacity: infoOpacity }]}>
            <Text style={styles.infoText}>Healthcare teams can securely manage and access patient records with consent.</Text>
          </Animated.View>
        </Animated.View>
      </ScrollView>
    </KeyboardAvoidingView>
...
const styles = StyleSheet.create({
  ...
  glow: { position: 'absolute', top: -100, left: '50%', marginLeft: -150, width: 300, height: 300, borderRadius: 150, backgroundColor: Colors.hospital },
  ...
  toggleActive: { backgroundColor: Colors.hospital },
  toggleText: { fontSize: Typography.body, color: Colors.textSecondary, fontWeight: Typography.medium },
  toggleTextActive: { color: '#fff', fontWeight: Typography.semibold },
  form: { flex: 1 },
  ...
  submitText: { fontSize: Typography.body, fontWeight: Typography.semibold, color: '#fff' },
