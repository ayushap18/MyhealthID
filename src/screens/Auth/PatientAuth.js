...
const PATIENT_IMAGE = 'https://images.unsplash.com/photo-1580281657521-5e38fa74d9e7?auto=format&fit=crop&w=640&q=80';
...
const PatientAuth = ({ navigation }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      logger.error('PatientAuth', 'Login error', error);
    ...
    try {
      ...
      if (data.wallet) {
        Alert.alert(
          ...
          [{ text: 'I Have Saved It', onPress: () => navigation.reset({ index: 0, routes: [{ name: 'PatientDashboard' }] }) }]
        );
      } else {
        navigation.reset({ index: 0, routes: [{ name: 'PatientDashboard' }] });
      }
    } catch (error) {
      logger.error('PatientAuth', 'Register error', error);
    ...
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        <Animated.View style={[styles.header, { opacity: fadeAnim, transform: [{ translateY: slideAnim }, { scale: headerScale }] }]}>
          <LinearGradient colors={['#00D9FF', '#0066FF']} style={styles.iconContainer}>
            <Animated.Image
          ...
          <Text style={styles.subtitle}>{isLogin ? 'Login to your account' : 'Create your health account'}</Text>
        </Animated.View>
        ...
        <Animated.View style={[styles.form, { transform: [{ translateX: formSlide }] }]}>
          ...
          <Animated.View style={{ transform: [{ scale: buttonScale }] }}>
            <TouchableOpacity style={styles.submitButton} onPress={handleSubmit} disabled={loading}>
              <LinearGradient colors={['#00D9FF', '#0066FF']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.submitGradient}>
          ...
          <Animated.View style={[styles.infoBox, { opacity: infoOpacity }]}>
            <Text style={styles.infoText}>Your health data stays encrypted end-to-end and anchored on blockchain for integrity.</Text>
          </Animated.View>
        </Animated.View>
      </ScrollView>
    </KeyboardAvoidingView>
...
const styles = StyleSheet.create({
  ...
  glow: { position: 'absolute', top: -100, left: '50%', marginLeft: -150, width: 300, height: 300, borderRadius: 150, backgroundColor: Colors.primary },
